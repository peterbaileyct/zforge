...
- **`get_neighbors`** returns the 1-hop graph neighbourhood of a known entity ID, optionally filtered by relationship or neighbour type. Use when an entity is already identified and the question is about its direct connections (see [Targeted Neighbour Traversal](RAG%20and%20GRAG%20Implementation.md#targeted-neighbour-traversal-get_neighbors)).
- **`find_path`** finds the shortest graph path between two known entity IDs up to a configurable depth. Use when `find_relationship` returns no direct or shared-neighbour results and the question concerns indirect connections (see [Path Query](RAG%20and%20GRAG%20Implementation.md#path-query-find_path)).
- **`get_source_passages`** retrieves raw source chunks for a known entity ID via graph traversal — no vector search. Use when verbatim grounding is needed for an entity already identified by another tool (see [Entity Source Passages](RAG%20and%20GRAG%20Implementation.md#entity-source-passages-get_source_passages)).
- All eight tools are produced by `graph_utils.make_world_query_tools(z_bundle_root, allowed_node_labels, embedding_connector)`, passing the Z-World's `allowed_nodes` list. The same factory is used by the Summarizer node in [World Generation](World%20Generation.md) and by world-querying agents in [Experience Generation](Experience%20Generation.md).
